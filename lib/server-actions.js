...
import { generateSlug } from "./utils" // Corrected import path for generateSlug
...
export async function saveNotionPost(notionData) {
  try {
    ...
    const postData = {
      ...
      excerpt: notionData.excerpt || notionData.title.substring(0, 150) + "...", // Use provided excerpt or generate
      ...
      ].filter(Boolean), // Include all relevant labels
      status: notionData.status,
      author: notionData.owner || "Yilung C",
      sub_topic: notionData.otherLifeLabels.length > 0 ? notionData.otherLifeLabels[0] : notionData.label, // Use first otherLifeLabel or main Label
      pinned: notionData.pinned,
      created_at: notionData.createdTime,
      updated_at: notionData.lastEditedTime,
      published_at: notionData.newPostDate || notionData.createdTime, // Use "New post date" if available
