...
import { generateSlug } from "./utils" // Import generateSlug from lib/utils.js
...
export async function getPostsByCategory(category, limit = 50) {
  ...
  // Common status filter - always applied
  ...
  let categorySpecificFilter = null // Initialize as null instead of empty object
  ...
  if (category === "book-reviews") {
    ...
    // Add specific filter for life-blog category
    categorySpecificFilter = {
      or: [
        {
          property: "人生其他",
        ...
        // Add more life-blog related labels as needed
        {
          property: "人生其他",
          multi_select: {
            contains: "Daily Life",
  ...
  // Build the combined filter properly
  let combinedFilter
  if (categorySpecificFilter) {
    ...
    // If no category-specific filter, just use the common status filter
...
export async function getPostBySlug(category, slug) {
  if (!NOTION_DATABASE_ID) {
    console.error("CRITICAL ERROR: NOTION_DATABASE_ID environment variable is not set.")
    return null // Return null instead of throwing to avoid build failures
  }
